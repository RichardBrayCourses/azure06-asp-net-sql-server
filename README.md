...
The SQL administrator password is not typed into the command line. On first deployment, `scripts/sql-password.sh` creates an Azure Key Vault for the environment, generates a password, and stores it in the `sql-admin-password` secret. Later deploy and migration commands reuse that secret.
...
GitHub Actions runs `deploy:<environment>` and then `<environment>:migrate:azure`. It uses Azure Key Vault for the SQL password, so no GitHub SQL password secret is required.
...
`AZURE_KEY_VAULT_NAME` defaults to `<appName>-kv`, for example `allcheckouttest-kv`.

`AZURE_SQL_ADMIN_PASSWORD_SECRET_NAME` defaults to `sql-admin-password`.
...
`scripts/deploy-infra.sh` calls:

```bash
AZURE_SQL_ADMIN_PASSWORD="$("$SCRIPT_DIR/sql-password.sh" ensure)"
```

`scripts/sql-password.sh ensure` creates the Key Vault if needed, grants the current Azure principal permission to `get`, `list`, and `set` secrets, creates `sql-admin-password` if missing, and prints the password to the deploy script.

`scripts/migrate-azure-sql.sh` calls:

```bash
AZURE_SQL_ADMIN_PASSWORD="$("$SCRIPT_DIR/sql-password.sh" get)"
```

That reads the existing secret. It fails if the environment has not been deployed yet.
...
Receives the SQL password from Key Vault. `@secure()` prevents Azure from logging the value as a normal deployment parameter.
...
Outputs the SQL administrator username. The password is never output; scripts read it from Key Vault.
...
1. Reads the SQL password from Key Vault.
2. Reads SQL server/database outputs from the Azure deployment.
3. Adds your current public IP to the SQL firewall as `AllowMigrationClient`.
4. Runs `dotnet ef database update` with an explicit Azure SQL connection string.
...
```bash
AZURE_SQL_CONNECTION_STRING="Server=tcp:...;Initial Catalog=...;User ID=...;Password=...;Encrypt=True;TrustServerCertificate=False;" \
...
Read the generated SQL password secret name:

```bash
DEPLOY_ENV=testing bash scripts/sql-password.sh get
```

That command prints the password. Use it only when you deliberately need the raw value.
